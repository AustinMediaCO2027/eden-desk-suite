...
export const LandingHero = () => {
  return (
    <section className="relative overflow-hidden pt-24 md:pt-32 pb-0 min-h-screen flex flex-col">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div
          className="absolute top-[10%] left-1/2 -translate-x-1/2 w-[900px] h-[500px] opacity-30"
          style={{ background: "radial-gradient(ellipse at center, hsl(var(--brand) / 0.35) 0%, transparent 70%)", filter: "blur(40px)" }}
        />
      </div>

      {/* Subtle grid */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[800px] h-[1px] bg-gradient-to-r from-transparent via-brand/30 to-transparent" />
      ...
      <div className="container mx-auto px-6 relative z-10 flex-1 flex flex-col items-center justify-center text-center">
        {/* Trust pill — avatars + count, mirrors reference hero badge */}
        <div className="inline-flex items-center rounded-full border border-border/50 bg-card/20 backdrop-blur-sm pl-2 pr-5 py-2 mb-10 animate-fade-in">
          <div className="flex -space-x-2 mr-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="w-7 h-7 rounded-full border-2 border-background"
                style={{ background: `linear-gradient(135deg, hsl(32 95% ${65 - i * 8}%) 0%, hsl(20 30% ${30 - i * 5}%) 100%)` }}
              />
            ))}
          </div>
          <span className="text-xs font-medium text-muted-foreground tracking-wide">
            <span className="text-foreground font-semibold">17,000+</span> businesses organized this week
          </span>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 animate-fade-in" style={{ animationDelay: "0.3s" }}>
          <Link to="/auth?mode=signup">
            <Button size="lg" className="text-base px-10 h-14 gap-2.5 bg-brand text-brand-foreground hover:bg-brand/90 rounded-xl font-semibold eden-brand-glow transition-all duration-300">
      ...
      <div className="relative mt-16 md:mt-24 animate-slide-up" style={{ animationDelay: "0.5s" }}>
        <div className="container mx-auto px-6">
          <div className="relative mx-auto max-w-5xl">
            <div className="absolute -inset-12 rounded-3xl" style={{ background: "radial-gradient(ellipse at center, hsl(var(--brand) / 0.06) 0%, transparent 70%)" }} />
