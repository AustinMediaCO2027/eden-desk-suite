...
const CreativeTemplate = ({ type, profile, documentNumber, date, dueDate, clientName, clientEmail, clientAddress, items, taxRate, notes }: TemplateProps) => {
      ...
      <div className="flex-1 p-8">
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">#{documentNumber}</p>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          ...
          <div className="rounded-lg p-4" style={{ backgroundColor: `${brandColor}08` }}>
            <p className="text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: brandColor }}>Bill To</p>
        ...
        <table className="w-full mb-6 text-sm">
          <thead>
            <tr>
              <th className="text-left py-2.5 text-[10px] font-bold uppercase tracking-widest border-b-2" style={{ color: brandColor, borderColor: brandColor }}>Description</th>
              <th className="text-center py-2.5 text-[10px] font-bold uppercase tracking-widest w-14 border-b-2" style={{ color: brandColor, borderColor: brandColor }}>Qty</th>
              <th className="text-right py-2.5 text-[10px] font-bold uppercase tracking-widest w-24 border-b-2" style={{ color: brandColor, borderColor: brandColor }}>Rate</th>
              <th className="text-right py-2.5 text-[10px] font-bold uppercase tracking-widest w-24 border-b-2" style={{ color: brandColor, borderColor: brandColor }}>Amount</th>
            </tr>
          </thead>
          <tbody>
            {items.map((item, i) => (
