...
export const CalendarView = ({ tasks, meetings, onAddTask, onEditTask, onToggleTask }: Props) => {
  ...
  const getTasksForDate = (dateStr: string) =>
    tasks.filter(t => t.due_date === dateStr || t.date === dateStr);
  ...
  const renderDayCell = (date: Date | null, minimal?: boolean) => {
    ...
    return (
      <div
        key={dateStr}
        className={`min-h-[80px] md:min-h-[100px] rounded-lg border p-1.5 cursor-pointer transition-colors hover:bg-accent/50 ${
          isToday ? "border-foreground/30 bg-accent/30" : "border-border"
        }`}
      ...
      >
        ...
        <div className="space-y-0.5">
          ...
          {dayMeetings.slice(0, 2).map(m => (
            <div key={m.id} className="text-[10px] truncate px-1 py-0.5 rounded bg-blue-500/10 text-blue-600 dark:text-blue-400 flex items-center gap-0.5">
              <Video className="h-2 w-2 shrink-0" />
              {m.title}
            </div>
          ))}
          {(dayTasks.length > 3 || dayMeetings.length > 2) && (
            <div className="text-[9px] text-muted-foreground px-1">+{dayTasks.length - 3 + dayMeetings.length - 2} more</div>
          )}
        </div>
      </div>
    );
    ...
    <div className="space-y-4">
      ...
      {viewMode === "month" && (
        <div>
          ...
          <div className="grid grid-cols-7 gap-1">
            {getMonthDays().map((d, i) => renderDayCell(d))}
          </div>
        </div>
      )}
      ...
      {viewMode === "week" && (
        <div>
          ...
          <div className="grid grid-cols-7 gap-1">
            {getWeekDays().map(d => renderDayCell(d))}
          </div>
        </div>
      )}
