...
/**
 * Template 3 – Classic with sidebar metadata
 * Matches Invoice_3.png: "INVOICE" bold top-left, logo top-right,
 * billed-to left / company right, card with sidebar metadata + bordered items table.
 */
const InvoiceTemplate3 = forwardRef<HTMLDivElement, InvoiceTemplateProps>(
  ({ profile, documentNumber, date, dueDate, clientName, clientEmail, clientAddress, items, taxRate, notes, colorOverride }, ref) => {
    ...
    return (
      <div ref={ref} style={PAGE} data-print-template="invoice" data-template-style="template3">
        {/* Top header bar */}
            ...
            <tbody>
              <tr>
                <td style={{ width: "50%", verticalAlign: "middle" }}>
                  <p style={{ margin: 0, fontSize: "7mm", fontWeight: 800, color: "#1a1a1a", letterSpacing: "0.3mm" }}>INVOICE</p>
                </td>
          ...
          <tbody>
            <tr>
              <td style={{ width: "50%", verticalAlign: "top" }}>
                <p style={{ margin: 0, fontSize: "2.6mm", color: "#888", textTransform: "uppercase", fontWeight: 600, letterSpacing: "0.2mm" }}>Billed to</p>
              ...
              <tr>
                ...
                <td style={{ width: "25%", verticalAlign: "top", padding: "5mm", borderRight: "0.3mm solid #e5e7eb" }}>
                  <div style={{ marginBottom: "4mm" }}>
                    <p style={{ margin: 0, fontSize: "2.5mm", color: "#888", fontWeight: 600 }}>Invoice #</p>
                    <p style={{ margin: "0.8mm 0 0", fontSize: "3mm", fontWeight: 600 }}>{safeText(documentNumber, 30)}</p>
                  </div>
                  <div style={{ marginBottom: "4mm" }}>
                    <p style={{ margin: 0, fontSize: "2.5mm", color: "#888", fontWeight: 600 }}>Invoice date</p>
                    <p style={{ margin: "0.8mm 0 0", fontSize: "3mm" }}>{safeText(date, 30)}</p>
                  </div>
                  <div style={{ marginBottom: "4mm" }}>
                    <p style={{ margin: 0, fontSize: "2.5mm", color: "#888", fontWeight: 600 }}>Reference</p>
                    <p style={{ margin: "0.8mm 0 0", fontSize: "3mm" }}>{safeText(documentNumber, 20)}</p>
                  </div>
                  <div>
                    <p style={{ margin: 0, fontSize: "2.5mm", color: "#888", fontWeight: 600 }}>Due date</p>
                    <p style={{ margin: "0.8mm 0 0", fontSize: "3mm", fontWeight: 600 }}>{safeText(dueDate, 30)}</p>
                  </div>
                </td>
                        ...
                        <td colSpan={4} style={{ padding: "2mm 4mm 2mm 0" }}>
                          <div style={{ backgroundColor: accent, borderRadius: "2mm", padding: "2.5mm 4mm" }}>
                            <table style={{ width: "100%", borderCollapse: "collapse" }}>
                              <tbody><tr>
                                <td style={{ fontSize: "3.2mm", fontWeight: 700, color: "white", textAlign: "left" }}>Total due</td>
                                <td style={{ fontSize: "3.2mm", fontWeight: 700, color: "white", textAlign: "right", whiteSpace: "nowrap", fontVariantNumeric: "tabular-nums" }}>{fmt(total)}</td>
                              </tr></tbody>
                            </table>
                          </div>
                        </td>
