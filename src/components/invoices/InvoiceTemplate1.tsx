...
/**
 * Template 1 – Executive / Freelancer
 * Matches Invoice_1.png: warm accent, logo+company top-left, big total top-right,
 * rounded card for billing/meta, items table, notes, terms footer.
 */
const InvoiceTemplate1 = forwardRef<HTMLDivElement, InvoiceTemplateProps>(
  ({ profile, documentNumber, date, dueDate, clientName, clientEmail, clientAddress, items, taxRate, notes, colorOverride }, ref) => {
    ...
    return (
      <div ref={ref} style={PAGE} data-print-template="invoice" data-template-style="template1">
        {/* Header: Logo+Company left, Client address right */}
            ...
            <tbody>
              <tr>
                <td style={{ width: "38%", verticalAlign: "top" }}>
                  <p style={{ margin: 0, fontSize: "2.6mm", color: "#888", textTransform: "uppercase", letterSpacing: "0.2mm" }}>Billed to</p>
                ...
                <td style={{ width: "30%", verticalAlign: "top" }}>
                  <p style={{ margin: 0, fontSize: "2.6mm", color: "#888" }}>Invoice number</p>
        ...
        <table style={{ ...T, marginBottom: "5mm" }}>
          <tbody>
            <tr>
              <td style={{ width: "40%" }}>
                <p style={{ margin: 0, fontSize: "2.6mm", color: "#888" }}>Subject</p>
                <p style={{ margin: "1mm 0 0", fontSize: "3.2mm", fontWeight: 600 }}>{safeText(notes, 40) || "Services"}</p>
              </td>
              <td style={{ width: "30%" }}>
                <p style={{ margin: 0, fontSize: "2.6mm", color: "#888" }}>Invoice date</p>
                <p style={{ margin: "1mm 0 0", fontSize: "3.2mm", fontWeight: 600 }}>{safeText(date, 30)}</p>
              </td>
              <td style={{ width: "30%", textAlign: "right" }}>
                <p style={{ margin: 0, fontSize: "2.6mm", color: "#888" }}>Due date</p>
                <p style={{ margin: "1mm 0 0", fontSize: "3.2mm", fontWeight: 600 }}>{safeText(dueDate, 30)}</p>
              </td>
            </tr>
          </tbody>
        </table>
        ...
        {notes && (
          <div style={{ marginBottom: "4mm" }}>
            <p style={{ margin: 0, fontSize: "3mm", fontWeight: 600 }}>Thanks for the business.</p>
          </div>
        )}
        ...
        <div style={{ position: "absolute", left: "20mm", right: "20mm", bottom: "20mm" }}>
          {profile?.bank_name && (
            <div style={{ marginBottom: "3mm" }}>
              <p style={{ margin: 0, fontSize: "2.6mm", color: accent, fontWeight: 600 }}>Banking details</p>
              <p style={{ margin: "0.8mm 0 0", fontSize: "2.7mm", color: "#666" }}>
                {safeText(profile.bank_name, 36)} • {safeText(profile.bank_account_holder, 36)} • Acc {safeText(profile.bank_account_number, 30)}
          ...
          {notes && (
            <div style={{ marginBottom: "3mm" }}>
              ...
              <p style={{ margin: "1mm 0 0", fontSize: "2.7mm", color: "#888", whiteSpace: "pre-wrap" }}>{safeText(notes, 3000)}</p>
            </div>
          )}
